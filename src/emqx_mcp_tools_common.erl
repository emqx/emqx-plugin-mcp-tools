...
get_log_messages(LogLevel0, MaxMsgs0, StartTime0, EndTime0) ->
    ...
    MsgOrder = case StartTime > EndTime of
        true -> reverse;
        false -> normal
    end,
    ...
    {LogMsgs, _} = read_logs_from_files(FilteredFiles, {[], 0}, LogLevel, StartTime, EndTime, MsgOrder, MaxMsgs),
    #{node_name => node(), log_messages => LogMsgs}.

%%==============================================================================
%% Read log files
%%==============================================================================
read_logs_from_files([#{file := File, log_type := LogType} | Files], {LogMsgs, LastLogTs}, LogLevel, StartTime, EndTime, MsgOrder, MaxMsgs) ->
    {ok, Fd} = file:open(File, [read, binary, raw, {read_ahead, ?READ_AHEAD}]),
    try
        {LogMsgs1, LastLogTs1} = read_lines_to_list(Fd, LogType, LogLevel, StartTime, EndTime, MsgOrder),
        {LogMsgs2, LastLogTs2} = case LastLogTs1 >= LastLogTs of
        ...
        case length(LogMsgs2) >= MaxMsgs of
            ...
            false ->
                read_logs_from_files(Files, {LogMsgs2, LastLogTs2}, LogLevel, StartTime, EndTime, MsgOrder, MaxMsgs)
        end
    catch
        throw:Reason ->
            ?SLOG(error, #{msg => read_log_failed, file => File, error => Reason}),
            read_logs_from_files(Files, {LogMsgs, LastLogTs}, LogLevel, StartTime, EndTime, MsgOrder, MaxMsgs);
        Err:Reason:St ->
            ?SLOG(error, #{msg => read_log_failed, file => File, error => Err, reason => Reason, stacktrace => St}),
            read_logs_from_files(Files, {LogMsgs, LastLogTs}, LogLevel, StartTime, EndTime, MsgOrder, MaxMsgs)
    after
        file:close(Fd)
    end;
    ...
    case file:read_line(Fd) of
        {ok, Line} ->
            case parse_line(Line, LogType) of
                {ok, #{time := Ts, level := Level, message := LogMsg}} when MsgOrder =:= normal, Ts > StartTime, Ts =< EndTime ->
                    case logger:compare_levels(Level, LogLevel) of
                        lt -> read_lines_to_list(Fd, LogType, LogLevel, StartTime, EndTime, MsgOrder, Acc);
                        _ -> read_lines_to_list(Fd, LogType, LogLevel, StartTime, EndTime, MsgOrder, {[LogMsg | LogMsgs], Ts})
                    end;
                {ok, #{time := Ts, level := Level, message := LogMsg}} when MsgOrder =:= reverse, Ts < StartTime, Ts >= EndTime ->
                    case logger:compare_levels(Level, LogLevel) of
                        lt -> read_lines_to_list(Fd, LogType, LogLevel, StartTime, EndTime, MsgOrder, Acc);
                        _ -> read_lines_to_list(Fd, LogType, LogLevel, StartTime, EndTime, MsgOrder, {[LogMsg | LogMsgs], Ts})
                    end;
...
parse_line(Line, text) ->
    try
        ...
        Ts = case is_numeric_str(TsStr) of
...
get_log_files() ->
    ...
    lists:foldl(
        fun (#{module := logger_disk_log_h, config := #{file := FilePrefix}, formatter := {Fmtr, _}}, Acc) ->
                LogType = case Fmtr of
                ...
                [begin
                    {ok, #file_info{size = Size, mtime = MtimeS}} = file:read_file_info(File, [{time, posix}]),
...
log_level(Level) ->
    throw({badlog_level, #{level => Level}}).
...
parse_time(Time) when is_binary(Time) ->
    try calendar:rfc3339_to_system_time(binary_to_list(Time), [{unit, microsecond}])
    catch
        error:Reason ->
...
parse_relative_time(Sign, OffsetStr) ->
    case re:run(OffsetStr, "^(\\d+)([smhdw])$", [{capture,all_but_first,binary}]) of
        {match, [NumStr, Unit]} ->
            Num =
                try binary_to_float(NumStr)
                catch
                    _:_ -> throw({bad_time_number, #{number => NumStr}})
                end,
            OffSetMs = case Unit of
                <<"s">> -> timer:seconds(Num);
                <<"m">> -> timer:minutes(Num);
                <<"h">> -> timer:hours(Num);
